function changeServices(e) {
  ...
  for (i = 0; i < inputs.length; i++) {
    if (inputs[i].type == "checkbox") {
      if (inputs[i].name.slice(-6) == "enable") continue;
...
function displayServices(data) {
  Object.keys(data).forEach(function(v) {
    ...
    if (el === null) el = document.querySelector('select[name="' + v + '"]');
...
function setMDNS(v) {
  ajaxSpin("POST", "/services/update?mdns_enable=" + (v ? 1 : 0),
           function() { showNotification("mDNS is now " + (v ? "enabled" : "disabled")); },
           function() {
...
function fetchServices() {
  ajaxJson("GET", "/services/info", displayServices,
           function() { window.setTimeout(fetchServices, 1000); });
}