...
function updateText(resp) {
  ...
  if (resp !== null && resp.len > 0) {
    ...
    el.innerHTML = el.innerHTML.concat(resp.text.replace(/&/g, '&amp;')
                                           .replace(/</g, '&lt;')
                                           .replace(/>/g, '&gt;')
                                           .replace(/"/g, '&quot;'));
...
function consoleSendInit() {
  ...
  sendHistory.addEventListener("change", function(e) { inputText.value = sendHistory.value; });
  ...
  function pushHistory(text) {
    ...
    var newOption = m('<option>' + (text.replace(/&/g, '&amp;')
                                        .replace(/</g, '&lt;')
                                        .replace(/>/g, '&gt;')
                                        .replace(/"/g, '&quot;')) +
                      '</option>');
  ...
  inputText.addEventListener("keydown", function(e) {
    switch (e.keyCode) {
      case 38:
        /* the up arrow key pressed */
        e.preventDefault();
        navHistory(-1);
        break;
      case 40:
        /* the down arrow key pressed */
        e.preventDefault();
        navHistory(+1);
        break;
      case 27:
